;; downloaded from
;; https://gitlab.com/chicken-sdl2/chicken-sdl2/-/blob/main/demos/basics.scm
;;
...
;;(set! (sdl2:window-maximum-size window) '(1920 1080))
...
(define (draw-text! text x y modifier)
  (define-values (w h) (ttf:size-utf8 font text))
  (cond
     ((eq? modifier 'gold)
      (sdl2:fill-rect! (sdl2:window-surface window)
		       (sdl2:make-rect (- x 10) (- y 5) (+ w 10) (+ h 5))
		       (sdl2:make-color 80 80 0))
      )
     ((eq? modifier 'hole)
      (sdl2:fill-rect! (sdl2:window-surface window)
		       (sdl2:make-rect (- x 10) (- y 5) (+ w 10) (+ h 5))
...
(define smiley3 (make-obj (make-smile-surf (make-random-color)) 150 150))
  ...
  (lambda ()
    (let* ((s *state*)
      ...
      (letrec ((foo (lambda (x y)
			...
			(foo 1 (+ y 1)))
		       ((> y hgt) #f)
		       (#t (let ((expr (d:xy s x y)))
			     (match expr
			       (('used used 'size size)
				;;(format #t "~a/~a " used size)
				(cond
				 ((and (= x hole-x)(= y hole-y))
				  (draw-text! (format #f "~a/~a" used size) (* 50 x)(* 25 y) 'hole))
				 ((and (= x gold-x)(= y gold-y))
				  (draw-text! (format #f "~a/~a" used size) (* 50 x)(* 25 y) 'gold))
				 (#t
...
(define (draw-scene!)
  (let ((window-surf (sdl2:window-surface window)))
    ...
    ;; Draw the smileys
    (draw-obj! smiley3 window-surf)
    (set! (obj-x smiley3) (+ 1 (obj-x smiley3)))
    (cond
     ((> (obj-x smiley3) 1800)
      (set! (obj-x smiley3) 0)))
